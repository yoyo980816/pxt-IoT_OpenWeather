...
namespace IoT_OpenWeather {
    ...
    let IoT_SERIAL_TX = SerialPin.P12
    let IoT_SERIAL_RX = SerialPin.P8
    let IoT_WIFI_SSID = ""
    ...
    */
    ...
    function IoT_connect_wifi(): void {
        if (IoT_SERIAL_INIT) {
            ...
            let item = "test"
            IoTWriteString("|2|1|" + IoT_WIFI_SSID + "," + IoT_WIFI_PASSWORD + "|\r") //Send wifi account and password instructions
            item = serial.readUntil("\r")
            while (item.indexOf("|2|3|") < 0) {
                item = serial.readUntil("\r")
            }
            IoT_IP = item.substr(5, item.length - 6)
    ...
    /**
     * Setup IoT Tx Rx to micro:bit pins and WIFI SSID, password.
     * 設定WIFI的Tx、Rx連接腳位，以及WIFI的名稱及密碼
     * @param SSID to SSID ,eg: "yourSSID"
     * @param PASSWORD to PASSWORD ,eg: "yourPASSWORD"
     * @param receive to receive ,eg: SerialPin.P8
     * @param send to send ,eg: SerialPin.P12
    */
    //% weight=100 group="01_System"
    //% receive.fieldEditor="gridpicker" receive.fieldOptions.columns=3
    //% send.fieldEditor="gridpicker" send.fieldOptions.columns=3
    //% blockId=IoT_WIFI_setup blockGap=5
    //% block="IoT setup WIFI | Pin set: | Receive data (green wire): %receive| Send data (blue wire): %send | Wi-Fi: | Name: %SSID| Password: %PASSWORD| Start connection"
    export function IoT_WIFI_setup(/*serial*/send: SerialPin, receive: SerialPin,
        ...
        IoT_SERIAL_TX = send
        IoT_SERIAL_RX = receive
        IoT_serial_init()
        IoT_connect_wifi()
    }
/*
    //% weight=99
    //% blockId=IoT_serial_disconnect
    //% block="IoT serial disconnect"
    export function IoT_serial_disconnect(): void {
        IoT_SERIAL_INIT = false
    }
    //% weight=98
    //% blockId=IoT_serial_reconnect
    //% block="IoT serial reconnect"
    export function IoT_serial_reconnect(): void {
        IoT_serial_init()
    }
*/
    /**
     * return the IP of your IoT 
     * 取得IoT的IP
    */ 
    //% weight=97 group="01_System"
    //% blockId=getIoT_IP blockGap=5
    //% block="get IoT IP address"
    export function getIoT_IP(): string {
        if (IoT_SERIAL_INIT && IoT_WIFI_CONNECTED)
            return IoT_IP
        else
            return ""
    }
